...
<head>
    ...
    $stmt = $conn->prepare("SELECT id,uuid,users,nodesid FROM servers WHERE uuid = ?");
    if ($stmt === false) {
    ...
    // Lier les paramètres
    $stmt->bind_param("s", $_COOKIE['user_uuid']);
    ...
    if ($stmt->num_rows > 0) {
        ...
        // Vérifier le mot de passe
            ...
            $stmt = $conn->prepare("SELECT ip , name FROM nodes WHERE id = ?");
            if ($stmt === false) {
                echo "Erreur de préparation de la requête : " . $conn->error;
                exit();
            }
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "http://$ip/server/". $_COOKIE['user_uuid'],
...
if (!isset($_SESSION["uuid"]) && !isset($_COOKIE['user_uuid'])) {
...
if (isset($_COOKIE['user_uuid'])) {
    $_SESSION["uuid"] = $_COOKIE['user_uuid'];
}
    ?>
    <script>
        function fetchData(action) {
            fetch(`http://127.0.0.1/module/power.php?action=${action}&uuid=<?php echo $_SESSION["uuid"]; ?>`)
...
<body>
    ...
    <div class="nav">
        ...
        <div class="el" onclick="window.location.href='../files'" ;>Fichier (EN BETA)</div>
    ...
    <main>
        ...
        <div class="support">
            ...
            Code de support<br>
            <?php echo  substr($_SESSION["uuid"], 0, 8)?> 
        </div>
