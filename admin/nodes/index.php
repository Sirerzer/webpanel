...
$stmt = $conn->query("SELECT ip,id,name,tags,ram,disk,server FROM `nodes` WHERE 1");
...
if ($stmt->num_rows > 0) {
    ...
    foreach ($rows as $row) {
        foreach ($row as $key => $cell) {
            if ($key == 'ip') {
                echo '                                <td class="text-center left-icon" data-action="ping" data-secret="O NON" data-location="http://'.$cell.'/node/status"><i class="fa fa-fw fa-heartbeat faa-pulse animated" data-original-title="" title="" style="color: rgb(80, 175, 81);"></i></td>';
